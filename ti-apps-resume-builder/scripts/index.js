...
function createCheckboxes(jsonData){
                ...
                jsonData.forEach((item,index) => {
                   ...
                   if(typeof item[index]!= 'undefined'){
                    if(typeof item[index].folder.split('/')[0] !='undefined' && item[index].folder.split('/')[0] != ''){
                         name = item[index].folder.split('/')[0]
                          collapseItem = `
                         <div class="card">
                             ...
                             <div id="collapse${index}" class="card-collapse collapse" aria-labelledby="heading${index}" data-bs-parent="#accordion">
                                 ...
                                 if(typeof item[index].contents[index] !='undefined'){
                                 item[index].contents.forEach((items=>{
                                     //debugger
                                     collapseItem+=`<li class="list-group-item"><input type="checkbox" onchange="addmdfiles(this)" name="checkbox" value="${item[index].path}${item[index].folder}${items}">${items}</li>`
                                 }))
                             }
                             collapseItem+=`</ul></div>
                             </div>
            ...
            function fetchMarkdownContent(file) {
                //debugger
                 return fetch('/ti-apps-resume-builder/mdcontent?file=' + encodeURIComponent(file))
            ...
            $.ajax({
                url: '/ti-apps-resume-builder/save',
                type: 'POST',
    ...
    $.ajax({
        url: '/ti-apps-resume-builder/draft',
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify(jsonData),
        success: function(response) {
...
$('#loadDraftButton').click(function() {
    $.ajax({
        url: '/ti-apps-resume-builder/load-draft',
        type: 'GET',
        success: function(response) {
            //debugger
...
async function loadfiles(file,counter,response){
 ...
 fetch('/ti-apps-resume-builder/jsondata')
 .then(response => response.json())
